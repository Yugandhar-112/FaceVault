...
> Powered by **ArcFace** embeddings + **FAISS-HNSW** search + **Differential Privacy** + **Homomorphic Encryption (PHE)**.
...
| Crypto backend | `crypto_backend.py` | Paillier Homomorphic Encryption (PHE) for encrypted similarity |
...
| Benchmarks | `benchmark.py` | P/R/F1, ε-accuracy, stability, timing, HNSW ablations, PHE tests |
| State persistence | `pickle` + `faiss.write_index` | Survive Streamlit re-runs |
| UI | Streamlit | Browser-based interface with premium, responsive design |
...
3. **Encryption** — Noised centroids are serialised via `msgpack` and encrypted. The default pipeline uses **Fernet** (AES-128-CBC) for speed. An alternative **Paillier Homomorphic Encryption (PHE)** backend (`crypto_backend.py`) provides mathematically secure inner products on encrypted vectors, preventing the relay server from ever seeing plaintext.

4. **Relay Comparison** — The Flask sync server receives manifests from multiple devices, computes pairwise **cosine similarity** across devices. Pairs exceeding the sync threshold (0.70) are flagged as merge suggestions.
...
> **⚠️ Semi-honest relay (Fernet mode).** In the default fast demo, the key travels with the ciphertext. The server decrypts for comparison but is trusted not to leak plaintext centroids.  
> **🔐 Fully-Blind Relay (PHE mode).** Using the Paillier backend, the relay computes similarities using homomorphic addition and scalar multiplication, returning an encrypted dot product. This guarantees that the relay never sees plaintext centroids.
...
Runs a comprehensive 6-step suite covering:
...
4. Encryption overhead timing (Fernet)
5. Encrypted similarity timing (Paillier Homomorphic Encryption)
6. HNSW parameter ablation (M and efSearch vs recall/speed)

Produces `benchmarks/results.json`, `epsilon_accuracy.png`, `centroid_stability.png`, and `hnsw_ablation.png`.
...
```
FaceVault/
├── app.py                # Main Streamlit app (premium UI, 3 tabs)
...
├── crypto_backend.py     # Paillier Homomorphic Encryption backend
├── sync_server.py        # Flask blind relay for cross-device comparison
├── merge.py              # Local merge logic with backup & rollback
├── benchmark.py          # 6-step evaluation suite (accuracy, DP, HNSW ablations, PHE)
├── requirements.txt      # Python dependencies
├── .gitignore            # Excludes large binaries and dataset
├── README.md
└── tests/
    ├── __init__.py
    ├── test_centroid.py       # 7 tests (EMA, fragmentation)
    ├── test_privacy.py        # 9 tests (DP noise, Fernet)
    ├── test_crypto_backend.py # 10 tests (PHE operations, accuracy)
    ├── test_sync_server.py    # 9 tests (Flask routes, comparison)
    └── test_merge.py          # 7 tests (Label extraction, merges)
```
